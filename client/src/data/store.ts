...
export function migrate(input: any): DbState {
  ...
  // 11 -> 12: ورديات الصندوق.
  if (version < 12) {
    state.cashSessions = state.cashSessions || [];
    version = 12;
  }

  // 12 -> 13: دفعات الأصناف بأرقام تشغيلات وصلاحيات مستقلة.
  if (version < 13) {
    state.batches = state.batches || [];
    version = 13;
  }

  // 13 -> 14: المستخدمون والصلاحيات وسجل التدقيق.
  if (version < 14) {
  ...
  // 17 -> 18: مستندات ما قبل البيع (عروض، أوامر، تسليم، معلّقة).
  if (version < 18) {
  ...
  // 19 -> 20: قوائم الأسعار، الأصول الثابتة، المصروفات المقدمة، مراكز التكلفة.
  if (version < 20) {
    state.priceLists = state.priceLists || [];
    state.fixedAssets = state.fixedAssets || [];
    state.prepaidExpenses = state.prepaidExpenses || [];
    state.costCenters = state.costCenters || [];
    version = 20;
  }

  // 20 -> 21: سجل البضاعة المتلفة.
  if (version < 21) {
